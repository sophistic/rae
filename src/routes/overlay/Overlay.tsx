...
import { Pin, Torus, X, Mic, Send, Pencil, Minimize2, Monitor } from "lucide-react";
	...
	if (!expanded) {
		...
		return (
			<div className="w-full h-full  flex items-center justify-center p-2 box-border">
				<main className={`w-full  h-full bg-white flex flex-col rounded-xl shadow-lg overflow-hidden min-h-0`}>
					{/* Header bar */}
					<div className={`flex items-center  w-full h-[44px] shrink-0 ${isPinned ? "" : "drag"}`}>
						<OverlayButton className="!border-none" customBgColor="white"  active={isActive} onClick={() => setIsActive(!isActive)} ><div className={`size-3 ${isActive ? "bg-green-500" : "bg-gray-700"} rounded-full`} ></div></OverlayButton>
						<div className="group drag  flex-1 h-full flex items-center w-full">
							{!showChat ? (
								inputActive ? (
									<div key="input-field" className="flex w-full h-full items-center border-x border-gray-300 px-4 py-2 drag bg-white  shadow-sm  max-w-xs">
										<input
											autoFocus
											type="text"
									...
									>
										<span className={`text-sm font-medium cursor-text z-50 text-zinc-800 ${inputText ? '' : 'text-gray-500'}`}>
											{inputText || 'Ask Quack anything...'}
										</span>
									</div>
								)
							) : (
								<div key="listening-field" className="flex drag items-center gap-2 px-4 py-2 text-sm text-gray-600">
									<span className="select-none font-medium">Listening to:</span>
									{windowIcon ? (
										<img src={windowIcon} alt="App icon" className="w-5 h-5 rounded-sm" />
									) : (
					...
					<div className="flex items-center h-full  ml-auto">
						...
						<OverlayButton onClick={handlePinClick} active={isPinned} title="Pin">
							<Pin size={16} />
						</OverlayButton>
						{showChat && (
							<OverlayButton onClick={handleCloseChatClick} className="no-drag hover:bg-gray-300 rounded p-2" title="Close chat">
								<X size={16} />
							</OverlayButton>
						)}
						{!showChat && (
							<OverlayButton onClick={handleFollowClick} className="no-drag hover:bg-gray-300 rounded p-2" title="Follow">
								<Minimize2 size={16} />
							</OverlayButton>
						)}
					</div>
				...
				<AnimatePresence initial={false}>
					{showChat && (
						<motion.div
							...
							className={`no-drag flex-1 flex flex-col overflow-hidden border-t border-gray-200 relative min-h-0 ${backgroundUrl && isAdjustingBg ? "cursor-move" : ""}`}
							style={
								backgroundUrl
									? {
							...
							<div className="flex-1 flex flex-col overflow-hidden bg-white/40 backdrop-blur-sm min-h-0">
								<div className="flex-1 overflow-y-auto px-6 py-4 space-y-2 scrollbar-hide">
									{messages.map((msg, idx) => (
										<div
											key={idx}
											className={`px-4 py-2 rounded-lg text-sm w-fit ${msg.sender === "user" ? "bg-gray-900 text-white self-end text-right ml-auto" : "bg-gray-200 self-start text-left"}`}
										>
											{msg.text}
										</div>
									))}
